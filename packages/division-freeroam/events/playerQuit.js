mp.events.add("playerQuit", async (player) => {
    ...
    await server.db.query('UPDATE `accounts` SET `level` = ?, `money` = ?, `group` = ?, `prisoned` = ? WHERE `Identity` = ?', [player.getLevel(), player.getMoney(), player.getGroup(), player.getVariable('prisoned'), player.identity]).then(() => {
