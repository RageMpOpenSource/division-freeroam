...
mp.events.add("playerJoin", (player) => {
    ...
    player.loadCharacter = async function() {
        await server.db.query('SELECT `Outfit` FROM `accounts` WHERE `Identity` = ?', [this.identity]).then(([rows]) => {
            this.customCharacter = rows[0]["Outfit"];
            this.applyCharacter();
        }).catch(err => {
            console.log(err);
    ...
    player.saveCharacter = async function() {
        await server.db.query('UPDATE `accounts` SET `Outfit` = ? WHERE `Identity` = ?', [JSON.stringify(this.customCharacter), player.identity]).then(() => {
            console.log(`${player.name}'s outfit has been updated`);
